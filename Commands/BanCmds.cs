...
namespace Cliptok.Commands
{
    public class BanCmds
    {
        ...
        public async Task BanSlashCommand(SlashCommandContext ctx,
            ...
            [Parameter("appeal_link"), Description("Whether to show the user an appeal URL in the DM")] bool appealable = true,
        ...
        .AsEphemeral(true);
            ...
            var userAlreadyBanned = (await ctx.Guild.GetBansAsync()).Any(x => x.User.Id == user.Id);
         ...
         [RemainingText, Description("The time and reason for the ban. e.g. '14d trolling' NOTE: Add 'appeal' to the start of the reason to include an appeal link")] string timeAndReason = "No reason specified.")
        {
            ...
            // Check if the user is already banned first. If they are, we will update the ban record (editban) instead of writing a new one
            var userAlreadyBanned = (await ctx.Guild.GetBansAsync()).Any(x => x.User.Id == targetMember.Id);
